...
struct
  ...
  fun evaluate (0: int)((a,b): alphabeta)(s: Game.state)(m: Game.move) = check_bounds((a,b))(s)(m)(Game.estimate(s))
    | evaluate (n)((a,b))(s)(m) = let
      ...
      val parti = floor(Real.fromInt(Seq.length(moveseq))*prune_percentage)
      val firsti = Seq.take(parti)(moveseq)
      val lasti = Seq.drop(parti)(moveseq)
    in
      if not(Game.status_eq(Game.status(s),Game.In_play)) then check_bounds((a,b))(s)(m)(Game.estimate(s))
      else (case search (n)(a,b)(s)(firsti)(lasti) of
              Pruned => Value(Pruned)
            | BestEdge(m',e) => check_bounds((a,b))(s)(m)(e))
    end

  and search (n: int)((a,b): alphabeta)(s: Game.state)(abseq: Game.move Seq.seq)(mmseq: Game.move Seq.seq) = 
    if Seq.length(abseq)=0 
    then let
      val results = (Seq.map(fn x => evaluate(n-1)((a,b))(Game.make_move(s,x))(x))(mmseq))
      val player = Game.player(s)
    in
      if Seq.exists(fn ParentPrune => true | _ => false)(results) then Pruned else
        let val valseq = Seq.map(fn Value(x)=>x)(results) in
          (case player of 
            Game.Maxie => value_for (s) (updateAB (s) (a,b) (Seq.reduce (maxalpha)(Pruned)(valseq)))
          | _          => value_for (s) (updateAB (s) (a,b) (Seq.reduce (minbeta)(Pruned)(valseq)))
          ) end
    end
    else
      let
        val firstmove = Seq.nth(0)(abseq)
        val remaining = Seq.drop(1)(abseq)
      in
        (case (evaluate (n-1)((a,b))(Game.make_move(s,firstmove))(firstmove)) of
            ParentPrune => Pruned
          | Value(v) => search (n)(updateAB(s)((a,b))(v))(s)(remaining)(mmseq) )
      end

  fun next_move (s:Game.state) = let
    val moveseq = Game.moves(s)
    val parti = floor(Real.fromInt(Seq.length(moveseq))*prune_percentage)
    val firsti = Seq.take(parti)(moveseq)
    val lasti = Seq.drop(parti)(moveseq)
  in
    (case (search (search_depth)((Pruned,Pruned))(s)(firsti)(lasti)) of
      BestEdge(m,x) => m)
  end 
